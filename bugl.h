...
namespace bu
{
	...
	template <typename T>
	GLuint loadModel(const std::string& path, size_t *vertexCount, std::string& currentLine)
	{
		...
		if (!file.is_open())
		{
			std::cout << "\nERROR: Cannot Find the Object's File.";
			return NULL;
		}
		while (!file.eof())
		{
		...
		*vertexCount = faces.size() * 3;

		{
			...
			if (!vboId)
			{
				std::cout << "\nERROR: Cannot Generate VBO Buffers.";
			}
